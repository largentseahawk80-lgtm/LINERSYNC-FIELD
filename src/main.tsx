...
function Dashboard({ constants, records, points, setTab }: { constants: ConstantData; records: QCRecord[]; points: AsBuiltPoint[]; setTab:(t:any)=>void }) {
  ...
  return <section className="page"><h2>Field Dashboard</h2><p className="muted">Seam Log, AutoFill, Mythos, As-Built, AR Vision, GPS/time, Last Logs, and export are active.</p>
    <div className="kpis"><Kpi label="Total Logs" value={records.length}/><Kpi label="Seam Logs" value={seams}/><Kpi label="As-Built Points" value={points.length}/><Kpi label="Locked" value={locked}/></div>
    <div className="card"><h3>Mythos Field Agent</h3><p>{nextMythosAction(constants, records)}</p><div className="button-row"><button onClick={()=>setTab('mythos')}><Brain size={16}/> Open Mythos</button><button onClick={()=>setTab('capture')}><Plus size={16}/> Open Tap Capture</button><button onClick={()=>setTab('asbuilt')}><Map size={16}/> Open As-Built</button></div></div>
...
function AsBuilt({points,setPoints,constants}:{points:AsBuiltPoint[];setPoints:(p:AsBuiltPoint[])=>void;constants:ConstantData}){
  ...
  return <section className="page"><h2>As-Built Map</h2><p className="muted">Now upgraded: tap the site canvas to place seam, panel, repair, DT, or photo points. Saved logs still drop points automatically.</p>
...
function Vision({constants,gps,captureGps,points}:{constants:ConstantData;gps?:GPSPoint;captureGps:()=>void;points:AsBuiltPoint[]}){
  ...
  return <section className="page"><h2>AR Vision HUD</h2><p className="muted">Upgraded: live camera feed with field overlay. Next step after this is pinning real GPS targets into the camera view.</p>
